...
fun EmailNotificationsScreen(
    ...
    Column(modifier = Modifier.fillMaxSize()) {
        SettingsHeader(
            title = stringResource(R.string.text_email_notifications_header),
        ...
        modifier = Modifier
            ...
            .clickable {
                viewModel.changeNotificationsMode(
                    notificationType = EMAIL_NOTIFICATIONS_TYPE,
        ...
        Column(
            modifier = Modifier
                .constrainAs(text) {
                    ...
                    if (viewModel.emailNotificationsMode.value == ALL_NOTIFICATIONS) {
        ...
        Column(
            modifier = Modifier
                .constrainAs(text) {
                    ...
                    bottom.linkTo(parent.bottom)
                    start.linkTo(parent.start)
                    if (viewModel.emailNotificationsMode.value == RELEASE_NOTIFICATIONS) {
                        end.linkTo(check.start, 8.dp)
                    } else {
                        end.linkTo(parent.end, 32.dp)
                    }
...
private fun DisableNotifications(viewModel: SettingsViewModel, notificationsArray: Array<String>) {
    Row(
        ...
        if (viewModel.emailNotificationsMode.value == DISABLE_NOTIFICATIONS) {
            Image(
                imageVector = ImageVector.vectorResource(R.drawable.ic24_action_done),
                contentDescription = stringResource(id = R.string.icon_arrow_right),
                colorFilter = ColorFilter.tint(xenon()),
